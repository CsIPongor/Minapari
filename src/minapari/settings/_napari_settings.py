...
from minapari._pydantic_compat import Field
from minapari.settings._appearance import AppearanceSettings
from minapari.settings._application import ApplicationSettings
from minapari.settings._base import (
    _NOT_SET,
    EventedConfigFileSettings,
    _remove_empty_dicts,
)
from minapari.settings._experimental import ExperimentalSettings
from minapari.settings._fields import Version
from minapari.settings._plugins import PluginsSettings
from minapari.settings._shortcuts import ShortcutsSettings
from minapari.utils._base import _DEFAULT_CONFIG_PATH
...
class NapariSettings(EventedConfigFileSettings):
    ...
    def _maybe_migrate(self):
        if self.schema_version < CURRENT_SCHEMA_VERSION:
            from minapari.settings._migrations import do_migrations
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 2:
        ...
        dest = Path(__file__).parent / 'minapari.schema.json'
