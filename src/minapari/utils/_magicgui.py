...
if TYPE_CHECKING:
    ...
    from minapari._qt.qthreading import FunctionWorker
...
def _get_layer_from_widget(gui: ComboBox, viewer: Viewer) -> Layer | None:
    ...
    if gui.annotation is None or gui.annotation.__module__ != 'minapari.types':
        return None
...
def add_layer_data_to_viewer(gui: FunctionGui, result: Any, return_type: type):
    ...
    """
    from minapari._qt._qapp_model.injection._qprocessors import (
...
def add_worker_data(
    ...
    widget : MagicGui
        ...
        dock widget.
    worker : WorkerBase
        An instance of `minapari._qt.qthreading.WorkerBase`, on worker.returned,
...
def add_future_data(gui, future: Future, return_type, _from_tuple=True):
    ...
    _from_tuple : bool, optional
        ...
        False if it returns one of the `LayerData` types.
    """
    from minapari._qt._qapp_model.injection._qprocessors import _add_future_data
...
def find_viewer_ancestor(widget) -> Viewer | None:
    ...
    from minapari.viewer import current_viewer
