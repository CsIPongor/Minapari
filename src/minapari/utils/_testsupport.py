...
def napari_plugin_manager(monkeypatch):
    ...
    """
    import napari
    import minapari.plugins.io
    from minapari.plugins._plugin_manager import NapariPluginManager
    ...
    with suppress(AttributeError):
        monkeypatch.setattr(minapari._qt.qt_main_window, 'plugin_manager', pm)
...
def _clean_themes():
    from minapari.utils import theme
...
def mock_app_model():
    ...
    This fixture must be used whenever `minapari._app_model.get_app_model()` is called to
    ...
    from minapari._app_model._app import NapariApplication, _napari_names

    try:
        from minapari._qt._qapp_model.qactions import init_qactions
        from minapari.plugins import _initialize_plugins
    except ImportError:
...
def make_napari_viewer(
    ...
    is_internal_test = request.module.__name__.startswith('minapari.')

    # disable thread for status checker
    monkeypatch.setattr(
        'minapari._qt.threads.status_checker.StatusChecker.start',
        _empty,
    )
    ...
    if 'enable_console' not in request.keywords:
        ...
        monkeypatch.setattr(
            'minapari._qt.qt_viewer.QtViewer._get_console', _dummy_widget
        )
...
def make_napari_viewer_proxy(make_napari_viewer, monkeypatch):
    ...
    """
    from minapari.utils._proxies import PublicOnlyProxy
