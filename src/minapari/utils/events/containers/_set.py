...
from minapari.utils.events import EmitterGroup
from minapari.utils.translations import trans

_T = TypeVar('_T')

if TYPE_CHECKING:
    from minapari._pydantic_compat import ModelField
...
class EventedSet(MutableSet[_T]):
    ...
    def validate(cls, v: Sequence, field: ModelField) -> EventedSet:
        """Pydantic validator."""
        from minapari._pydantic_compat import sequence_like
        ...
        for i, v_ in enumerate(v):
            ...
            if error:
                errors.append(error)
        if errors:
            from minapari._pydantic_compat import ValidationError
